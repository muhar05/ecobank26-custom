    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
            <form method="POST" action="{{ route('community-cash.contributions.update', $contribution) }}">
                ...
                <div class="mb-4">
                    <label for="member_id" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Pilih Warga Terdaftar</label>
                ...
                <div class="mb-4">
                    <label for="member_name" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Atau isi Nama Warga Manual</label>
                    <input type="text" name="member_name" id="member_name" value="{{ old('member_name', $contribution->member_name) }}" placeholder="Isi jika warga belum terdaftar" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Gunakan nama manual jika warga belum terdaftar.</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $contribution->amount) }}" required min="1" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $contribution->date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('date') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label for="description" class="block text-sm font-medium text-slate-700 dark:text-slate-200">Keterangan</label>
                    <input type="text" name="description" id="description" value="{{ old('description', $contribution->description) }}" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
