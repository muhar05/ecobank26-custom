<x-layouts.dashboard title="Tambah Kategori Dana">
    <div class="w-full mx-auto py-8">
        <!-- Page Header -->
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Kategori Dana</h1>
            <p class="mt-2 text-slate-600 dark:text-slate-400">Buat kategori baru untuk mengelompokkan pemasukan dan pengeluaran kas</p>
        </div>
            ...
            <form method="POST" action="{{ route('community-cash.categories.store') }}">
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative" x-data="{ raw: '{{ old('target_amount') }}', get display() { return this.raw ? new Intl.NumberFormat('id-ID').format(this.raw) : '' } }">
                ...
                <div x-data="{ submitting: false }" x-init="$el.closest('form').addEventListener('submit', () => { submitting = true })" class="border-t border-slate-200 dark:border-slate-700 px-8 py-6 flex items-center justify-between">
                    ...
                    <button type="submit" :disabled="submitting" 
                        ...
                        <span x-text="submitting ? 'Menyimpan...' : 'Simpan Kategori'"></span>
                    </button>
                </div>
            </form>
        </div>
