        ...
        <form method="POST" action="{{ route('community-cash.categories.store') }}">
            ...
            <x-form-section>
                ...
                <x-field-group label="Target Dana" name="target_amount" helper="Opsional">
                    <x-rupiah-input name="target_amount" :value="old('target_amount')" placeholder="0" />
                </x-field-group>
                ...
                <div x-data="{ isMandatory: {{ old('is_mandatory', false) ? 'true' : 'false' }} }" class="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700 mt-4">
                    <div class="flex items-start gap-3">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="is_mandatory" id="is_mandatory" value="1" @change="isMandatory = $event.target.checked" {{ old('is_mandatory', false) ? 'checked' : '' }} 
                                class="w-5 h-5 text-emerald-600 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded focus:ring-emerald-500 focus:ring-2 mt-0.5">
                        </div>
                        <div>
                    ...
                    <div x-show="isMandatory" style="display: none;" x-transition class="mt-3">
                        <x-field-group label="Nominal Iuran Bulanan (Rupiah)" name="monthly_amount" required>
                            <x-rupiah-input name="monthly_amount" :value="old('monthly_amount')" placeholder="Contoh: 20000" />
                        </x-field-group>
                    </div>
                </div>
            </x-form-section>
