<x-layouts.dashboard title="Import Data Anggota Warga V2">
    <div class="space-y-6">
        <x-form-card title="Import Data Anggota Warga V2" description="Upload file Excel (.xlsx) atau CSV (.csv) untuk meng-import data Anggota Warga secara massal dan menghubungkannya dengan Kartu Keluarga (KK).">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <div class="mt-2 text-sm text-emerald-700 dark:text-emerald-300 space-y-1">
                                    <p>✓ {{ $r['created'] }} data Anggota Warga baru berhasil ditambahkan.</p>
                                </div>
                                @if(isset($r['stats']))
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                @if($r['has_failed_download'])
                                    <div class="mt-4">
                                        <a href="{{ route('members.import-v2.failed-download') }}" class="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold px-4 py-2 rounded-lg shadow transition">
            ...
            <form method="POST" action="{{ route('members.import-v2.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <x-form-section title="1. Download Template Excel" description="Unduh template Excel dengan multi-sheet (Sheet 1: Data, Sheet 2: Petunjuk).">
                    <div class="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-slate-900 dark:text-slate-100">template_anggota_warga.xlsx</p>
                                <p class="text-xs text-slate-500 dark:text-slate-400">Dilengkapi format header bold, indikator kolom wajib, dan mapping ke KK yang ada.</p>
                            </div>
                        </div>
                        <a href="{{ route('members.import-v2.template') }}" class="inline-flex items-center justify-center bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800 transition px-4 py-2 rounded-lg text-sm font-semibold shadow-sm">
                            Download
                        </a>
                    </div>
                </x-form-section>
                ...
                <x-form-actions cancelUrl="{{ route('members.index') }}" submitLabel="Import Anggota Warga" />
            </form>
        </x-form-card>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div class="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                <h3 class="text-base font-bold text-slate-900 dark:text-white">Riwayat Import Terakhir (Warga V2)</h3>
