<x-layouts.dashboard title="Data Warga">
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
            <form method="GET" action="{{ route('members.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3">
                <div class="lg:col-span-2">
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari warga..." class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm h-11 dark:text-white dark:bg-slate-800 dark:border-slate-600">
                </div>
                <div>
                    <select name="rt_id" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm h-11 dark:text-white dark:bg-slate-800 dark:border-slate-600">
                        <option value="">Semua RT</option>
                        @foreach($rts as $rt) <option value="{{ $rt->id }}" {{ ($rtFilter ?? '') == $rt->id ? 'selected' : '' }}>RT {{ $rt->rt_number }}</option> @endforeach
                    </select>
                </div>
                <div>
                    <select name="gender" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm h-11 dark:text-white dark:bg-slate-800 dark:border-slate-600">
                ...
                <div>
                    <select name="status" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm h-11 dark:text-white dark:bg-slate-800 dark:border-slate-600">
                ...
                <div>
                    <select name="age_category" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm h-11 dark:text-white dark:bg-slate-800 dark:border-slate-600">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="w-full bg-emerald-600 text-white px-4 rounded-lg text-sm font-medium h-11 dark:bg-emerald-500">Filter</button>
                    <a href="{{ route('members.index') }}" class="w-full bg-slate-100 text-slate-700 px-4 rounded-lg text-sm font-medium h-11 flex items-center justify-center dark:bg-slate-800 dark:text-slate-200">Reset</a>
                </div>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    <thead class="bg-slate-50 dark:bg-slate-800">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Warga</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Gender</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Tgl Lahir / Umur</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">RT</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Status</th>
                            <th class="px-6 py-4 text-center text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Aksi</th>
                        </tr>
                    </thead>
