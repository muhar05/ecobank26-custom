<x-layouts.dashboard title="Import Data Warga">
    <div class="max-w-2xl space-y-6">
        <div>
            <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-50">Import Data Warga</h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Upload file CSV untuk menambah atau memperbarui data warga/nasabah secara massal.</p>
        </div>
        ...
        @if(session('import_result'))
            ...
            <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-4 space-y-2">
                ...
                <ul class="text-xs text-emerald-700 dark:text-emerald-400 space-y-1">
                    <li>✓ {{ $r['created'] }} warga baru ditambahkan</li>
                    <li>✓ {{ $r['updated'] }} warga diperbarui</li>
                    @if(count($r['errors']))
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="p-4 sm:p-6 space-y-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <a href="{{ route('members.import.template') }}" class="inline-flex items-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                        Download Template
                    </a>
                </div>

                <form method="POST" action="{{ route('members.import.store') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
            ...
            <div class="px-4 sm:px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700">
                ...
                <ul class="text-xs text-slate-500 dark:text-slate-400 space-y-1">
                    <li>• Data warga/nasabah dipakai bersama oleh Kas RT/RW dan Bank Sampah</li>
                    <li>• Kolom <code class="bg-slate-200 dark:bg-slate-700 px-1 rounded">name</code> wajib diisi</li>
                    <li>• Kosongkan <code class="bg-slate-200 dark:bg-slate-700 px-1 rounded">member_code</code> jika ingin digenerate otomatis (WRG001, WRG002, ...)</li>
                    <li>• Nomor HP membantu mencegah data double (duplikat dideteksi via HP atau kode)</li>
                    <li>• Jika member_code atau phone cocok dengan data yang sudah ada, data akan diperbarui</li>
                </ul>
            </div>
        </div>

        <a href="{{ route('members.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition">
