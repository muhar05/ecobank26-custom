                ...
                init() {
                    ...
                    if (oldRows && Object.keys(oldRows).length) {
                        Object.values(oldRows).forEach(d => {
                            ...
                            if (cat) this.rows.push({ catId: cat.id, name: cat.name, unit: cat.unit, price: Number(d.price_per_unit) || 0, weight: Number(d.weight) || 0, memberPrice: 0, hasPrice: false, manualOverride: !!Number(d.price_per_unit) });
                        });
                    }
