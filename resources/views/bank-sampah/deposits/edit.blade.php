<x-layouts.dashboard title="Edit Setoran Sampah">
    <div x-data="depositForm()" x-cloak>
        <x-form-card title="Edit Setoran Sampah"><div class="p-6">
            ...
            <form method="POST" action="{{ route('bank-sampah.deposits.update', $deposit) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $deposit->date->format('Y-m-d')) }}" required class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm text-sm focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="notes" value="{{ old('notes', $deposit->notes) }}" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 shadow-sm text-sm focus:border-emerald-500 focus:ring-emerald-500">
                </div>
    ...
    <script>
        function depositForm() {
            ...
            const existingDetails = @json($deposit->details->map(fn($d) => ['waste_category_id' => $d->waste_category_id, 'weight' => (float)$d->weight, 'price_per_unit' => (float)$d->price_per_unit]));
            return {
                ...
                init() {
                    ...
                    source.forEach(d => {
                        ...
                        if (cat) this.rows.push({ catId: cat.id, name: cat.name, unit: cat.unit, price: Number(d.price_per_unit) || 0, weight: Number(d.weight) || 0, hasPrice: false, manualOverride: true });
                    });
