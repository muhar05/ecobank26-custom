<x-layouts.dashboard title="Data Pengepul">
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Data Pengepul</h2>
            <a href="{{ route('bank-sampah.collectors.create') }}" class="bg-emerald-700 dark:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-emerald-800 dark:hover:bg-emerald-400 transition">Tambah Pengepul</a>
        </div>
        ...
        <x-table-toolbar :search="$search ?? ''" placeholder="Cari pengepul..." />
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse($collectors as $collector)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                                <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-slate-100">{{ $collector->name }}</td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{{ $collector->phone ?? '-' }}</td>
                        ...
                        @empty
                            <tr><td colspan="4" class="px-6 py-4 text-center text-slate-500 dark:text-slate-400">Belum ada data pengepul.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
