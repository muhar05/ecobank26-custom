<x-layouts.dashboard title="Import Kategori Sampah">
    <x-form-card title="Import Kategori Sampah" description="Upload file CSV untuk menambah atau memperbarui kategori sampah secara massal.">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <div class="mt-2 text-sm text-emerald-700 dark:text-emerald-300 space-y-1">
                            <p>✓ {{ $r['created'] }} kategori baru ditambahkan</p>
                            <p>✓ {{ $r['updated'] }} kategori diperbarui</p>
                            @if(count($r['errors']))
            ...
            <x-form-section title="1. Download Template" description="Gunakan template CSV ini agar format data sesuai dengan sistem.">
                <div class="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-slate-900 dark:text-slate-100">template_kategori_sampah.csv</p>
            ...
            <x-form-section title="Panduan Pengisian" description="Beberapa hal yang perlu diperhatikan saat mengisi data.">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                        ...
                        <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-1">Nama Spesifik</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Gunakan nama yang jelas seperti "Botol Putih Bersih", "Botol Putih Kotor", dll.</p>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                        ...
                        <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-1">Satuan Opsional</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Kolom <code class="bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded text-slate-700 dark:text-slate-300">unit</code> opsional. Secara default akan diisi dengan <code class="bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded text-slate-700 dark:text-slate-300">kg</code> jika dikosongkan.</p>
                    </div>
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                        <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mb-3 font-bold text-sm">3</div>
                        <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-1">Pembaruan Satuan</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Jika nama kategori sudah terdaftar, hanya informasi satuan yang akan diperbarui.</p>
                    </div>
                </div>
            </x-form-section>

            <x-form-actions cancelUrl="{{ route('bank-sampah.waste-categories.index') }}" submitLabel="Import Data" />
        </form>
    </x-form-card>
</x-layouts.dashboard>
