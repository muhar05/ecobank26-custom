<x-layouts.dashboard title="Kas Bank Sampah">
    <div class="space-y-6">
        ...
        <div id="table-section" x-data="{ tableLoading: false }" @click="if($event.target.closest('nav[role=\'navigation\'] a') || $event.target.closest('a.page-link')) tableLoading = true" class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors duration-300 relative">
            {{-- Loading Overlay --}}
            ...
            <div class="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                <form method="GET" action="{{ route('bank-sampah.cash-report') }}" x-data="{ isSubmitting: false }" @submit="isSubmitting = true" class="flex flex-col sm:flex-row items-end gap-4 w-full">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <button type="submit" :disabled="isSubmitting" :class="{'opacity-75 cursor-not-allowed': isSubmitting}" class="w-full sm:w-auto h-10 bg-emerald-600 dark:bg-emerald-500 text-white px-5 rounded-lg text-sm font-semibold hover:bg-emerald-700 dark:hover:bg-emerald-400 transition shadow-sm">
