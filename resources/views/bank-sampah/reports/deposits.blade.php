<x-layouts.dashboard title="Laporan Setoran Sampah">
    <div class="space-y-6">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Laporan Setoran Sampah</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Pantau rincian setoran sampah nasabah ke bank sampah secara riil</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('bank-sampah.reports.deposits.print', request()->query()) }}" target="_blank" class="inline-flex items-center gap-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm transition">
                    <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                    Cetak / Simpan PDF
                </a>
                <a href="{{ route('bank-sampah.reports.deposits.excel', request()->query()) }}" class="inline-flex items-center gap-1.5 bg-emerald-600 dark:bg-emerald-500 text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-emerald-700 dark:hover:bg-emerald-400 shadow-sm hover:shadow transition">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-sm">
                ...
                <p class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{{ number_format($totalTransactions, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-sm">
                <p class="text-xs text-emerald-600 dark:text-emerald-400 font-semibold uppercase tracking-wide">Total Berat</p>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-sm">
                <p class="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">Total Nilai Setoran</p>
                <p class="text-2xl font-bold text-slate-900 dark:text-white mt-1">Rp {{ number_format($totalAmount, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 flex flex-col justify-between shadow-sm">
                <p class="text-xs text-amber-600 dark:text-amber-400 font-semibold uppercase tracking-wide">Rata-rata Setoran</p>
                <p class="text-2xl font-bold text-slate-900 dark:text-white mt-1">Rp {{ number_format($averageTransaction, 0, ',', '.') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 transition-colors duration-300">
            <form action="{{ route('bank-sampah.reports.deposits') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div>
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50 dark:bg-slate-800/50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-right class text-xs font-semibold text-slate-500 uppercase">Berat</th>
                            <th class="px-6 py-4 text-right class text-xs font-semibold text-slate-500 uppercase">Harga</th>
                            <th class="px-6 py-4 text-right class text-xs font-semibold text-slate-500 uppercase">Subtotal</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($details as $d)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-slate-400">
                                    {{ $d->deposit->date ? $d->deposit->date->toDateString() : '-' }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400 text-sm">
                                    Tidak ada data setoran dalam periode ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="block sm:hidden divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                @forelse($details as $d)
                    <div class="p-4 space-y-2">
                        ...
                        <div class="flex justify-between text-xs">
                            <span class="text-slate-500">Berat / Harga:</span>
