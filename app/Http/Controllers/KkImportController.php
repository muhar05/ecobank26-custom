...
class KkImportController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        // Clean keys to handle potential spaces or header formatting
        $cleanedRows = [];
        foreach ($rows as $row) {
            $cleanedRow = [];
        ...
        // Validate headers - ensure critical columns exist (case-insensitive)
        ...
        if (!array_key_exists('rt_number', $firstRow) || !array_key_exists('family_head', $firstRow) || !array_key_exists('status', $firstRow)) {
            return back()->withErrors(['file' => 'Format template salah atau kolom utama tidak ditemukan. Pastikan header sesuai template (rt_number, family_head, status).']);
        }
        ...
        foreach ($cleanedRows as $row) {
            ...
            // Handle number formatting issues where Excel strips leading zeros
            $rtNumber = $rtNumberRaw !== '' ? str_pad($rtNumberRaw, 3, '0', STR_PAD_LEFT) : null;
            
            // Clean KK Number of scientific notation and ensure it's treated as string
            ...
            $validator = Validator::make([
                ...
                'family_head.required' => 'Family Head wajib diisi.',
                'status.required' => 'Status wajib diisi.',
                'status.in' => 'Status harus: active, kontrak, pindah, atau kosong.',
            ...
            if ($kkNumber) {
                ...
                if ($exists) {
                    $stats['duplicates']++;
                    $stats['failed']++;
                    $err = 'No KK sudah terdaftar di database.';
                ...
                foreach ($validRows as $vr) {
                    if ($vr['kk_number'] === $kkNumber) {
                        $stats['duplicates']++;
                        $stats['failed']++;
                        $err = 'No KK duplikat di dalam file import.';
                        $errors[] = "Baris {$rowNum}: {$err}";
                        $failedRows[] = [
                            $row['rt_number'] ?? '',
                            $row['kk_number'] ?? '',
